...
<link rel="stylesheet" type="text/css" href="<?php echo GG_PATH;?>/css/style.css"/>
		...
		<!--[if lte IE 7]><style>.main{display:none;} .support-note .note-ie{display:block;}</style><![endif]-->				
				<div class="gamma-container gamma-loading" id="gamma-container">
					...
					<?php 
    $photos = get_children( array('include' => $include,'post_parent' => get_queried_object_id(), 'post_status' => 'inherit', 'post_type' => 'attachment', 'post_mime_type' => 'image', 'order' => $order, 'orderby' => $orderby) );
	if ($photos) {$ct=0;
		foreach ($photos as $photo => $i) {$ct++;?>
<li>
							<div data-alt="img<?php echo $ct;?>" data-description="<h3><?php echo get_the_title($photo); ?> </h3>" data-max-width="1800" data-max-height="2400">
								<div data-src="<?php $image_attributes = wp_get_attachment_image_src( $photo, 'large');echo $image_attributes[0]; ?>" data-min-width="1300"></div>
								<div data-src="<?php $image_attributes = wp_get_attachment_image_src( $photo, 'large');echo $image_attributes[0]; ?>" data-min-width="1000"></div>
								<div data-src="<?php $image_attributes = wp_get_attachment_image_src( $photo, 'large');echo $image_attributes[0]; ?>" data-min-width="700"></div>
								<div data-src="<?php $image_attributes = wp_get_attachment_image_src( $photo, 'medium');echo $image_attributes[0]; ?>" data-min-width="300"></div>
								...
								<div data-src="<?php $image_attributes = wp_get_attachment_image_src( $photo, 'thumbnail');echo $image_attributes[0]; ?>"></div>
								<noscript>
